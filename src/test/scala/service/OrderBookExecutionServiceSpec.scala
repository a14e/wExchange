...
class OrderBookExecutionServiceSpec
  extends FlatSpec
    with Matchers {


  "OrderBook.merge" should "add buy order to empty order book" in new Wirings {
    ...
    service.executeWithOrder(initBook, order) shouldBe(resultBook, resultOperations)

  }

  it should "add sell order to empty order book" in new Wirings {
    val initBook = OrderBook(
      pair = pair,
      asks = immutable.Seq.empty,
      bids = immutable.Seq.empty
  ...
  it should "add buy order to order book where no buys and no matches" in new Wirings {
    ...
    val result = service.executeWithOrder(initBook, order)
    result shouldBe(resultBook, resultOperations)
  }


  it should "add sell order to order book where no sells and no matches" in new Wirings {
  ...
  it should "execute equals orders with input sell order" in new Wirings {
    val existenBuyOrder = defaultBuyOrder.copy(
      price = 3,
      amount = 2
    )
    ...
    val (resultBook, resultOperations) = service.executeWithOrder(initBook, order)
    resultBook shouldBe expectedBook
    resultOperations.sortBy(_.amount) shouldBe expectedOperations.sortBy(_.amount)
  }


  it should "execute equals orders with input buy order" in new Wirings {
    val existenSellOrder = defaultSellOrder.copy(
      price = 3,
      amount = 2
    )
  ...
  it should "put sell order in valid place" in new Wirings {
  ...
  it should "put buy order in valid place" in new Wirings {
  ...
  it should "execute more then one buy order" in new Wirings {
  ...
  it should "execute more then one sell order" in new Wirings {
  ...
  it should "partially execute buy order and place left to bids" in new Wirings {
    val firstSellOrder = defaultSellOrder.copy(
      price = 2,
      amount = 2
    )
  ...
  it should "partially execute sell order and place left to asks" in new Wirings {
    val firstBuyOrder = defaultBuyOrder.copy(
      price = 3,
      amount = 2
    )
  ...
  trait Wirings {
    val service = new OrderBookExecutionServiceImpl
  }
